...
# counting the number of trees in each neighborhoods.
...
# Remove the geometry
# By using st_set_geometry on tree_counts to NULL, the geometry column is removed.
...
# Taking head of neigh and trees to see which column they have in common
...
neighborhoods_counts <- mutate(neighborhoods_counts, 
                            tree_cnt = ifelse(is.na(tree_cnt), 
                                              0, tree_cnt)) #If tree_cnt is na replace with 0.

head(neighborhoods_counts)

# Compute the density
neighborhoods_counts <- mutate(neighborhoods_counts, 
                               tree_density = tree_cnt/area) #calculating the tree density by deviding the number of trees in every nta by the area.
```
...
#As we need a to look at both tree_density and avg_canopy, I add them to the same dataframe. This is done by overwritning the old neigborhoods df.
neighborhoods <- neighborhoods %>% 
  cbind(avg_canopy = c(neighborhoods_avg_canopy$avg_canopy)) %>% #adding avg_canopy, from the neighborhoods_avg_canopy df.
  cbind(tree_density = c(neighborhoods_counts$tree_density)) #adding tree_density from neighborhoods_counts
head(neighborhoods)
...
map2 <- tm_shape(neighborhoods) + 
    tm_polygons(c("avg_canopy", "tree_density"),
    palette = "Greens", 
    style = "quantile", n = 7, 
    title = c("Average tree canopy in procent", "Trees pr. km^2"))
