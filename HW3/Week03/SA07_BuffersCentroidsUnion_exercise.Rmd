...
# Convert the data frame to an sf object. Dette gør man med st_as_sf. dvs. denne funktion konverterer en data-frame til et sf objekt.       
df_sf <- st_as_sf(df, coords = c("longitude", "latitude"), crs = 4326) #puts it into a differen format, specified coordinate system

# Transform the points to match the manhattan CRS. Her transformeres sf-objektet til at have samme crs som manhattan data settet.
...
# Confirm that we went from 8 features to 1 feature
length(beech)
length(beech1)

# Compute the tight bounding box
beech_hull <- st_convex_hull(beech1) #Kan lave enhver form
