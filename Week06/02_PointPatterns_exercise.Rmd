...
#EBT: Kest multidistance moving function.
#EBT: K-function: hvor clustered eller dispurst er de i et eller anden

#MB: so inside the envelope is insignificant variance from the random distribution? So the G function shows how likely you are to find the nearest neighbor within a given radius, and the K function is how many how are likely to find within that radius?
...
#EBT: Man kan sammenligne estimatet af K, ved at plotte formlen "- pi * r ^ 2 ~ r". Dog kun hvis data'en er genereret ud fra en poisen proces. Linjen skal være tæt på 0 for alle r-værdier???
```
...
#Hypotese: points (træerne) er clustered.
#Test om hypotesen passer imod en null hypotese af tilfældig uniform fordeling.
plot(redoak)
```
