...
This assignment is made in collaboration with Rasmus Vesti.
...
#We can use bw_choice$h to see all the bandwith and 'hcv'to see the best bandwidth

# Print the best bandwidth
print(bw_choice$hcv)
#From this it can be read that band with is 800
```
...
seg10 <- spseg(
    pts = preston_crime, 
    h = bw_choice$h, #"h" is chosen as our parameter for bandwidth
    opt = 3,
    ntest = 10, #10: the sample for the dataset
    proc = FALSE)
...
prob_violent <- list(x = seg$gridx,
                     y = seg$gridy,
                     z = matrix(seg$stpvalue[, "Violent crime"],
                                ncol = ncol))
